...
Branch `6.0` — package version `6.0`. Every method of `Csgt\Crud\CrudController`
...
| `edit(Request $request, $aId)` | `GET /resource/{id}/edit` | Renders `csgtcrud::edit` for an existing record: editable fields, combo options, breadcrumb, and which JS widgets the form needs. |
...
| `update(Request $request, $aId)` | `PUT /resource/{id}` | Creates when `$aId === 0`, updates otherwise. Filters out the ignored fields, merges the hidden ones, normalises each value by field type (dates through Carbon, uploads moved, `securefile` put on its disk and the previous one deleted, `bool` cast to 1/0), saves, then detaches/attaches the `multi` relations. Returns JSON or a redirect. |
...
| `getLocalEditFields()` | Fields with `editable => true` that are not relation columns; drives the edit form. |
...
| `setField($aParams)` | Declares one field. Accepts `field`, `name`, `editable`, `show`, `type`, `class`, `default`, `validationRules`, `validationRulesMessage`, `decimals`, `collection`, `enumarray`, `filepath`, `filewidth`, `fileheight`, `filedisk`, `target`, `isforeign`, `utc`, `editClass`. Types: `string`, `multi`, `numeric`, `date`, `datetime`, `time`, `bool`, `combobox`, `password`, `enum`, `file`, `image`, `textarea`, `url`, `summernote`, `securefile`. Rejects unknown keys. |
...
| `setOrderBy($aParams)` | Default order. |
| `setPerPage($aCuantos)` | Rows per page (default 50). |
| `setHidden($aParams)` | Values injected on save without being rendered. |
| `setignoreFields($aCampo)` | Request keys never persisted (`_token` is always ignored). |
| `setBreadcrumb($aArray)` | Custom breadcrumb trail. |
| `setExtraButton($aParams)` | Extra button per row (`url`, `icon`, `title`, `class`, `target`, `confirm`). |
| `setExtraAction($aParams)` | Extra action next to the Add button. |
