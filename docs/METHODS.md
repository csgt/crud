...
Branch `5.7` — package version `5.7`. Every method of `Csgt\Crud\CrudController`
...
| `update(Request $request, $aId)` | `PUT /resource/{id}` | Creates when `$aId === 0`, updates otherwise. Strips `ignoreFields` with `array_except()`, merges the hidden fields, normalises each value by field type (dates through Carbon, uploads moved, `securefile` put on its disk and the previous one deleted), saves, then detaches/attaches the `multi` relations. Returns JSON or a redirect. |
...
Private filters over `$fields`, the list built by `setField()`. Method names
are Spanish (`campos` = fields) even though the field array keys themselves are
English.
...
| `getLocalEditFields()` | Fields with `editable => true` whose `field` has no `.`; drives the edit form. |
...
| `setJoin($aRelation)` | Recorded into `$this->joins`, but `data()` never reads it — has no observable effect on the listing on this branch. |
