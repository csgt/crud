...
adding a brand new method. This branch is a Beta with known-unfinished parts
(see `ARCHITECTURE.md`, "What is unfinished") — finishing one of those parts is
also allowed without a new version, as long as it does not change the shape of
anything that already works (for example, the paginator JSON `data()` already
returns must keep returning that shape).
...
bounds of the branch you are on — this branch is `PHP >= 7.2`, `Laravel >= 7.0`
...
|---|---|---|
| `array_except()` / `str_slug()` | removed in Laravel 6, well below this branch's floor | `Arr::except()`, already used on this branch |
| `Relation::getRelationExistenceQuery()` without a guard | Laravel 5.5+; safe unguarded on this branch's floor, but this branch does not use it at all — ordering by a relation column is unimplemented, do not add it back without a test | guard with `method_exists()` on any branch older than `5.5` if you port this |
| `Model::qualifyColumn()` | Laravel 5.5+, safe on this branch | `$model->getTable().'.'.$column` only needed on branches older than `5.5` |
| `Paginator::withQueryString()` | Laravel 7.0+, exactly this branch's floor — do not port a fix using it to a branch below `7.0` without a fallback | build the query string manually on older branches |
| `[$a, $b] = $array` | PHP 7.1+, safe on this branch | only a concern on branches with a PHP floor below `7.1` |
