...
This branch (`master`, package version `8.0`) requires PHP `>= 7.2` and
Laravel `>= 5.8` (no upper bound declared; shipped against 8.x) — see
`README.md`'s "Requirements" section for the current numbers, since they are
what `README.md` states, not what is repeated here.
...
| `BelongsTo::getOwnerKeyName()` | Laravel 5.8+ | guard with `method_exists()` and fall back on branches with a lower floor |
| `Paginator::withQueryString()` | Laravel 7.0+ | build the query string manually (this branch does, in `getQueryString()`) on branches with a lower floor |
| `Arr::` / `Str::` | Laravel 5.5+ | the global helpers on branches that predate it |
| `array_except()` / `str_slug()` | removed in Laravel 6 | `Arr::except()` on branches that allow it (this branch already uses `Arr::except()`) |
...
(`composer test` runs `phpunit`, per the `scripts.test` entry in
`composer.json`; `vendor/bin/phpunit` works identically since `phpunit.xml`
...
- [ ] `composer test` is green, and the change is covered (rule 4)
