...
only.** Do not port new behaviour here from `5.3` or later; if you need a
feature this branch lacks, use `5.3` or newer instead. See `ARCHITECTURE.md`
for what this branch actually is (a static `Crud` class, no
`CrudController`) before touching anything.
...
- rename a method, a field key, a config key, a translation key or a view
  variable
- change the JSON shape returned by `getData()`
...
branch. Open it on a new version branch instead — see rule 2. On this archived
branch, in practice this means: only fixes, never new behaviour.

**Allowed without a new version:** fixing a method that is provably broken (it
throws, produces invalid SQL, or contradicts its own documented purpose), and
adding a new optional parameter with a default that preserves the current
result. Adding a brand new method is allowed on maintained branches, but on
this archived branch prefer opening it on `5.3` or newer instead, unless the
fix genuinely cannot be expressed any other way.
...
bounds of the branch you are on — this branch is `PHP >= 5.5`, Laravel
`5.1 - 5.2` — and before using a framework API, confirm it exists in the
lowest supported Laravel version.

Concrete traps that have already bitten this package:

| Do not use on this branch | Because | Use instead |
|---|---|---|
| `extra.laravel.providers` package auto-discovery | requires Laravel >= 5.5, above this branch's `5.1 - 5.2` floor | keep documenting manual registration in `config/app.php`, as the README already does |
| `[$a, $b] = $array` | requires PHP 7.1+, well above this branch's `>= 5.5` floor | `list($a, $b) = $array` or index access |
| `Arr::` / `Str::` facades | Laravel 5.5+ | the global helpers (`array_get()`, `str_slug()`, ...) |
| `array_except()` / `str_slug()` as global helpers | fine on this branch, but removed in Laravel 6 — do not assume they exist if you ever port code from a `5.3+` branch backward | keep the global helper form here; do not import `Arr::except()` |
| `Model::qualifyColumn()`, `Relation::getRelationExistenceQuery()` | need Eloquent relations, and this branch has none — `Crud` is a static class over `DB::table()` | there is nothing to port here; a "relation" on this branch is just a `leftJoin` + aliased column |
| `Storage::disk()->temporaryUrl()` | Laravel 5.4+, above this branch's floor | not used on this branch; do not add it |
...
touching `getData()` or its helpers ships with a test in `tests/` asserting the
...
`getBindings()` where a query is under test, never execute the query. If a
test fails, first check whether the source really behaves differently on this
branch — adjust the source or the expectation deliberately, never weaken an
assertion to get to green. `getData()` itself needs a booted application and
is out of scope for this suite (see `METHODS.md`); a targeted fix to it still
needs its own regression coverage, even if that means adding an
application-booting test harness the current suite deliberately avoids.

Filtering, ordering and pagination happen **in the database**. Do not
introduce `->get()` followed by Collection `filter`/`sortBy`/`splice`: that
loads the whole table into memory and the cost grows with the table instead
of the page.
...
- [ ] The change is a fix, not new behaviour — this branch is archived (see the
      note at the top of this file)
- [ ] No existing method changed name, signature or observable behaviour (rule 1)
- [ ] If behaviour had to change, it belongs on `5.3` or newer, not here (rule 2)
