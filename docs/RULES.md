...
oldest PHP and Laravel this branch supports. This branch's floor is PHP `>= 7.0`
and Laravel `>= 5.5` (see `README.md`); before using a framework API, confirm it
exists in Laravel 5.5.
...
| `BelongsTo::getOwnerKeyName()` | Laravel 5.8+ | avoid on code shared with the 5.5-5.7 branches |
| `Paginator::withQueryString()` | Laravel 7.0+ | avoid on this branch |

`Relation::getRelationExistenceQuery()` (Laravel 5.5+) is used directly and
without a `method_exists()` guard in `applyOrderToQuery()` — safe here because
this branch's floor is already Laravel 5.5. Do not copy that unguarded call into
the `5.3` or `5.4` branches, whose floor is lower; those branches guard it.
