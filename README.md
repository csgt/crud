...
> **You are on branch `5.8` - package version `5.8`.**
...
```bash
composer require csgt/crud:^5.8
```
...
Branch `5.8` is `5.7` with the `Arr` helper class instead of the removed
`array_except()` helper, which is what makes it usable on Laravel 7 and up.
