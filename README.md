...
> **You are on branch `5.7` - package version `5.7`.**
...
```bash
composer require csgt/crud:^5.7
```
...
This branch still calls the global `array_except()` helper, removed in Laravel
6, and `BelongsTo::getOwnerKeyName()`, added in Laravel 5.8. That pins it to
Laravel 5.8. Use branch `5.8` for Laravel 7 and up.
